...
NC="\e[0m"
COLOR1="$(cat /etc/samvpn/theme/$colornow | grep -w "TEXT" | cut -d: -f2|sed 's/ //g')"
COLBG1="$(cat /etc/samvpn/theme/$colornow | grep -w "BG" | cut -d: -f2|sed 's/ //g')"

APIGIT=$(cat /etc/samvpn/github/api)
EMAILGIT=$(cat /etc/samvpn/github/email)
USERGIT=$(cat /etc/samvpn/github/username)
...
if [[ -f /etc/samvpn/github/api && -f /etc/samvpn/github/email && /etc/samvpn/github/username ]]; then
   rec="OK"
else
    mkdir /etc/samvpn/github > /dev/null 2>&1
fi
...
sleep 2
echo "$EMAIL1" > /etc/samvpn/github/email
echo "$USERNAME1" > /etc/samvpn/github/username
echo "$API1" > /etc/samvpn/github/api
echo "ON" > /etc/samvpn/github/gitstat
clear
echo -e "$COLOR1┌─────────────────────────────────────────────────┐${NC}"
...
function resetipvps(){
clear
rm -f /etc/samvpn/github/email
rm -f /etc/samvpn/github/username
rm -f /etc/samvpn/github/api
rm -f /etc/samvpn/github/gitstat
echo "OFF" > /etc/samvpn/github/gitstat
echo -e "$COLOR1┌─────────────────────────────────────────────────┐${NC}"
...
echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}"
echo -e "$COLOR1┌─────────────────────────────────────────────────┐${NC}"
GITREQ=/etc/samvpn/github/gitstat
if [ -f "$GITREQ" ]; then
    cekk="ok"
else 
    mkdir /etc/samvpn/github
    touch /etc/samvpn/github/gitstat
    echo "OFF" > /etc/samvpn/github/gitstat
fi

stst1=$(cat /etc/samvpn/github/gitstat)
if [ "$stst1" = "OFF" ]; then
clear
echo -e "$COLOR1┌─────────────────────────────────────────────────┐${NC}"
echo -e "$COLOR1 ${NC} ${COLBG1}               • REGISTER IPVPS •              ${NC} $COLOR1 $NC"
...
stst=$(cat /etc/samvpn/github/gitstat)
if [ "$stst" = "ON" ]; then
APIOK="CEK API"
rex="viewapi"
else
